...
## 身体指标驱动规则
1. 必须根据身高、体重、BMI 与蛋白目标调整餐食结构，不要只按固定模板出餐。
2. 若 BMI 偏高或肥胖范围，优先选择高蛋白、高纤维、低 GI 主食、少油烹调与晚餐低负担组合，并避免用高糖饮品或高脂调味补热量。
3. 若 BMI 偏低，避免过度制造热量缺口，适当保留优质碳水和健康脂肪，并在 guardrails 或 mealPrepGuide 中提示不建议激进减脂。
4. 每日总蛋白应尽量落在主用户蛋白目标区间；若目标热量过低导致难以满足，需在 mealPrepGuide 或 guardrails 中说明权衡。`;
}
